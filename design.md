...
- **Natural Height Expansion**: The `onclone` handler overrides the cloned `#resume-print-area` styles to `height: auto` and `min-height: auto`. It also clears `overflow: hidden` and `height: 100%` on `clonedDoc.documentElement` and `clonedDoc.body`, forcing them to grow vertically to wrap all pages.
...
- **Ancestor Scroll Override**: Print media overrides `overflow: visible !important` and `height: auto !important` on `html`, `body`, and all ancestor wrapper `div` containers (targeted using `div:has(.resume-print-container)`) to restore standard multi-page browser pagination.
- **Outer border/Shadow Removal**: Print media overrides `.resume-print-container` and `#resume-print-area` to force `border: none !important`, `outline: none !important`, and `box-shadow: none !important`. This completely prevents grey borders, page outlines, or page-break shadows from rendering on fallback print PDFs.
